...
%     where W>=0 and H>=0 elementwise. nnmf
%
% Reference:
...
function [W,H,iter,HIS]=nmf(A,k,varargin)
    ...
    if par.verbose          % collect information for analysis/debugging
        [gradW,gradH] = getGradient(A,W,H,par.type,par.alpha,par.beta);%wh是随机值
    ...
    for iter=1:par.max_iter
        switch par.type
            case 'plain'
                ...
                gradH = (W'*W)*H - W'*A;
            case 'regularized'
                [H,gradHX,subIterH] = nnlsm([W;sbetaI],[A;zerokn],H,par.nnls_solver);%固定W，求H
